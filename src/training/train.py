...
def load_data(features_dir: str) -> pl.DataFrame:
    print("Loading features ...")
    df = pl.read_parquet(f"{features_dir}/features.parquet")
    print(f"  {len(df):,} rows")

    # Drop rows where any feature or label is null
    needed = FEATURE_COLS + [LABEL_COL, "game_id"]
    df = df.drop_nulls(subset=needed)
    print(f"  {len(df):,} rows after dropping nulls")

    # XGBoost needs numeric input — cast bool columns to int
    bool_cols = ["time_pressure_flag", "bishop_pair", "is_check", "is_castling", "is_promotion"]
...
def split_by_game(df: pl.DataFrame, test_fraction: float = 0.2) -> tuple:
    """Split by game_id not by row — prevents data leakage."""
...
def run(features_dir: str, models_dir: str) -> None:
    print("── Loading data ─────────────────────────────────")
...
if __name__ == "__main__":
    ...
    run(args.features_dir, args.models_dir)